...
var EPLAN=[
 {m:'Август', tag:'Фундамент', items:[
   ['⬜','Подать LLC «M5 Studio Miami» — неделя 3–9 августа','Florida LLC, Алекс 50% / Влад 50%. Регистрация + EIN. Это старт всего E-2 кейса. Решение 02.08: подаём на этой неделе.'],
...
var ALEXTODO=[
 ...
 ['todo','JobTread: тур на 40 минут ДО онбординг-звонка','Звонок запрошен у JobTread 1 авг: вторник 4 или среда 5 авг, 19:00 Дубай (=10:00 Central, =11:00 у Вадима). Холд в календаре стоит, ждём подтверждения. До звонка пройди демо-джоб «DEMO Turnkey Brickell 2BR» — гайд: M5/4 Рабочие документы/JobTread_первая_сессия.md; главное собрать тестовую смету и отправить её себе как клиенту. Каталог смет в аккаунте пока дефолтный от JobTread, наших услуг нет — это работа Вадима на звонке.'],
...
(function(){
  try{
    ...
    h+='<div class="lsn"><b>Правило по размеру файла — выбирать папки не нужно.</b><br>• <b>Фото и короткое видео</b> (до минуты) → прямо в @m5miami_bot, подпиши строкой к какому объекту: «Brickell гостиная — процесс», «объект Марии — после», «отзыв клиента».<br>• <b>Длинное видео</b> → в одну папку <b>00 Входящее</b> (ссылка ниже) + строка боту, что залил.<br>По папкам объектов раскладываю я. С телефона искать «правильную папку» не надо — там легко ошибиться, а потом кадры не найти. То же правило закреплено в «M5 Team». Большое видео (длиннее пары минут) грузи сразу в Drive → <b>00 Входящее</b> и напиши боту строку, что это было.</div>';
    ...
    h+='<div class="stk-g" style="margin-top:12px">Один объект — одна папка</div>';
    h+='<div class="lsn"><b>Никогда не сваливай два объекта вместе</b> — через месяц не разберёшь, где чья стена. Имя папки: <b>ГОД-МЕСЯЦ — Район или адрес — что делали</b>. Например: <i>2026-09 — Brickell, NE 1st Ave — венецианка в гостиной</i>. Внутри — До · Процесс · После · Детали и макро · Видео (скопируй шаблон).</div>';
    h+='<div class="lsn"><b>Клиентский объект (есть договор)</b> — контент идёт НЕ сюда, а в <b>02 Projects → имя клиента → 02 Before / 03 Progress / 06 After</b>. Правило: есть договор — живёт в проекте клиента; нет договора (портфолио, шоу-рум, своя работа) — здесь, в 01 Content.</div>';
    h+='<a class="stk" href="'+LINKS.objTemplate+'" target="_blank" rel="noopener"><b>📐 _ШАБЛОН объекта — копируй под новый</b><span>готовая структура папок: 01 До · 02 Процесс · 03 После · 04 Детали и макро · 05 Видео</span></a>';
    h+='<a class="stk" href="'+LINKS.vadimWorks+'" target="_blank" rel="noopener"><b>🧱 Работы Вадима до M5 — архив портфолио</b><span>сюда прошлые объекты: фото стен, процесс, видео — пригодится для сайта и рекламы</span></a>';
...
/* ═══ Инструкции по ролям — прямо в кабинете (просьба Алекса 31.07: не в Drive, текстом).
...
var ROLEGUIDE={
 ...
 smm:[
  ['1 · Фотосессия «ДО» — 3 дня после договора','Увидела в Projects «Договор подписан» → едешь на объект: фото + видео каждой комнаты → Drive: 02 Projects → [проект] → 02 Before. Из этого потом собираем «до/после».'],
  ['2 · Сырьё — только из Drive','01 Content (общий контент) и 03 Progress (Вадим грузит с объектов ежедневно). Ничего не снимаем «в никуда» — всё в папки.'],
  ...
  ['4 · Финал проекта','Фотосессия «ПОСЛЕ» → 06 After → рилс «до/после» — главный формат продаж.']],
 pm:[
  ['1 · Днём — фото с объекта','Фото/видео хода работ → Drive: 02 Projects → [проект] → 03 Progress. С рабочей почты @m5miami.com.'],
  ['2 · Вечером — 2 минуты','Голосовое или пару строк в «M5 Team» → Projects: что сделали · что завтра · что нужно. Всё — отчёт сдан.'],
  ['3 · Материалы на согласование','Фото образца → 04 Materials + строка в вечернем отчёте. Статус клиенту выставит Клод.'],
  ['4 · Галочки — не твоя забота','Прогресс в кабинете клиента обновляет Клод/Джин по твоему отчёту.']],
 designer:[
  ['1 · Концепт и мудборды','Ведёшь в JobTread + 3D и материалы в Drive проекта. Референсы стиля — Playbook.'],
  ['2 · Образцы материалов','Фото → Drive: 04 Materials — Клод выставит клиенту на согласование в кабинете.'],
...
var TEAMVIEW=[
 ...
 ['Бухгалтер','QuickBooks: инвойсы, оплаты, налоги · мы только складываем чеки в 04 Finance']
];
(function(){
  try{
